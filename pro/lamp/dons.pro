...
common c_dondo, bas,nbd
	if xregistered('DoCmd') eq 0 then begin
		...
		if idx(0) ge 0 then begin
		   ...
		   for i= 0,n_elements(maclist)-1 do bid=widget_button(bil,value=maclist(i),uvalue=[-88,225,i,bil,llg,bas,bir],font=ft_b_normal)
		endif
	endif
...
@dons.cbk
	...
	if n_elements(contain) le 19    then $
	box=widget_base  (bax,/column)  else $
	box=widget_base  (bax,/column,y_scroll=600)
...
;**		--- ----- -- --------- --- ----		
		if strpos(strcompress(lwtxt,/remove_all),'=transpose(') gt 0 then begin
...
curr_macr=''
n_emacs  =0
if prx then mac_list ,n_emacs, THISFILE='*.prox' $
       else mac_list ,n_emacs
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
pro mac_list,n_emacs ,mac_ful_out,macros_out, thisfile=thisfile
;** ********
;**
@lamp.cbk
@dons.cbk
...
;**	Lamp_macro macros
	...
	if n_elements(thisfile) eq 1 then begin
		...
		idx=where(mac_ful ne '')
		if idx(0) ge 0 then begin
			mac_ful_out=mac_ful(idx) & macros_out=macros(idx)
			for i=0,n_elements(idx)-1 do begin
				t=strpos(macros_out(i),'_')+1
...
;	Check for splitxyz
		...
		if (chbrk eq '(') or (chbrk eq '[') then $
		 if three gt 0 then pe=(strpos(line_2,chprbk,po3)+1)>po3 else $
		 if ok3   eq 0 then begin
		   f2p=strpos(line_2,':',po3)
		   lp =strpos(line_2,chprbk,po3)

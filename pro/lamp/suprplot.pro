...
common sc_supordid ,s_xylv,s_xylw       ; For XY limits and there widgets + log10
...
XMin = Min(s_x0,Max=XMax)		; Default Values for XMin, XMax
...
; gjk touched here!!!
  ...
  YMimi = YMin & if s_xylw(4) then YMimi=alog10(YMimi>1E-9)
  ...
  if s_xylw(4)    THEN YMinP = YMinP>0
...
if psp then begin if pop eq 0 then bcol=ccol
endif
...
FOR i=1,6 do begin
	...
	IF not(s_hid(i)) and not(s_hid2(i)) THEN BEGIN ; and not hided workspace
	   ...
	   ; this is to cancel the effect of YRange modified by s_yscale
	    ...
	    st  = 'OPLOT, s_x'+si+',s_lvd ,color=c, PSym=psymb, linestyle='+si2+',thick=lwth'
	    if (s_color) then c=i+2 else c=ccol
	    v=n_elements(s_lvd) & vl=400
	    IF psp THEN lwth=3 else if v gt vl then lwth=1 else lwth=2
	    res = execute(st)
	    if (s_color) and (v le vl*(1+4*psp)) then begin c=2 & lwth=1
		...
		if not s_xylw(4) then begin
			...
			st  = 'ERRPLOT, s_x'+si+', s_lvd'+opp1+ ', s_lvd'+opp2
			res = EXECUTE(st)
			!P.COLOR=keepcol
		endif
	    ENDIF
...
IF s_siz(0) ne 0 THEN $
	...
	OPLOT,s_x0,s_lvd ,Thick=1+s_color,color=c, PSym=7*s_psymb
	...
	IF s_show_err THEN BEGIN
		...
		if not s_xylw(4) then begin
			...
			st  = 'ERRPLOT, s_x0, s_lvd'+opp1+ ', s_lvd'+opp2
			res = EXECUTE(st)
			!P.COLOR=keepcol
		endif
	ENDIF
...
if xregistered('SUPER_PLOT') le 0 then BEGIN	; If Super_Plot doesn't exist..
    ...
    s_xylv=fltarr(4) & s_xylw=lonarr(5)		; For XY limits + Log10
    ...
    sw_sxmin	=WIDGET_SLIDER(sw_b33,Title='Y0   X_min' ,Minimum=0,Maximum=499,/Drag, $

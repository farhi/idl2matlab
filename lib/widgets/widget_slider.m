...
%UNUSED : drag, scroll, title?
    ...
    I2Mkwv=    {'base'   , 'not_r'          , 'eventp'    , 'eventf'     , 'f_get'          , 'p_set'         , 'map' , 'val'   , 'uval'   , 'font' , 'xsize' , 'ysize' , 'scr_xs'    , 'scr_ys'    , 'sens'      , 'uname' , 'resource_name' , 'vert'     , 'drag' , 'maxim'   , 'minim'   , 'suppress'       , 'title' , 'I2M_pos' };
    base=[]; not_r=[]; eventp=[]; eventf=[]; f_get=[]; p_set=[]; map=[]; val=[]; uval=[]; font=[]; xsize=[]; ysize=[]; scr_xs=[]; scr_ys=[]; sens=[]; uname=[]; resource_name=[]; vert=[]; drag=[]; maxim=[]; minim=[]; suppress=[]; title=[]; I2M_pos=[];

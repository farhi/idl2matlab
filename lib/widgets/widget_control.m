...
%TO DO   : clear_ev, motion, set_table_select
    ...
    I2Mkwv=    {'base'   , 'bad'    , 'timer' , 'not_r'          , 'use_tab'          , 'edit_cell' , 'eventp'    , 'eventf'     , 'f_get'          , 'p_set'         , 'getu'       , 'getv'      , 'setu'       , 'setv'      , 'destr'   , 'map' , 'realz'   , 'sens'      , 'reset' , 'show' , 'focus'       , 'defont'       , 'send_event' , 'kill_notify' , 'notify_realize' , 'but_ev'             , 'get_draw'      , 'hourglass' , 'uname'     , 'editable' , 'draw_xsize' , 'draw_ysize' , 'scr_xsize' , 'scr_ysize' , 'xsize' , 'ysize' , 'set_list_top' , 'set_list_sel'    , 'set_text_sel'    , 'set_text_top'      , 'append' , 'clear_ev'     , 'set_but'    , 'set_drop_sel'        , 'sl_min'         , 'sl_max'         , 'xoffset' , 'yoffset' , 'tlb_get_offset' , 'tlb_set_xoffset' , 'tlb_set_yoffset' , 'tlb_set_title' , 'tlb_get_size' , 'leader'       , 'I2M_pos'};
    base=[]; bad=[]; timer=[]; getu=[]; getv=[]; not_r=[]; use_tab=[]; edit_cell=[]; eventp=[]; eventf=[]; f_get=[]; p_set=[]; setu=[]; setv=[]; destr=[]; map=[]; realz=[]; sens=[]; reset=[]; show=[]; focus=[]; defont=[]; send_event=[]; kill_notify=[]; notify_realize=[]; but_ev=[]; get_draw=[]; hourglass=[]; uname=[]; editable=[]; draw_xsize=[]; draw_ysize=[]; scr_xsize=[]; scr_ysize=[]; xsize=[]; ysize=[]; set_list_top=[]; set_list_sel=[]; set_text_sel=[]; set_text_top=[]; append=[]; clear_ev=[]; set_but=[]; set_drop_sel=[]; sl_min=[]; sl_max=[]; xoffset=[]; yoffset=[]; tlb_get_offset=[]; tlb_set_xoffset=[]; tlb_set_yoffset=[]; tlb_set_title=[]; tlb_get_size=[]; leader=[]; I2M_pos=[];
...
if ~isempty(base), try,
if base > 0; fii=double(int16(base)); fig=I2Mfig(fii,3);
if fig  > 0; top=I2Mfig(I2Mfig(fii,11),1);
  ...
  if realz;
          ...
          set(fig,'position',[40,40,I2Mfig(fii,9:10)+4]);
          if I2Mfig(fii,5); set(fig,'visible','on'); end;
  ...
  if ~isempty(scr_xsize),  if typ=='axes'; bose=I2Mfig(fii,2); else bose=base;  end;  bos=int16(bose);
                                                                pos=I2Mfig(bos,7:10); pos(3)=scr_xsize;  I2Mfig(bos,7:10)=pos; set(bose,'position',pos); I2Mfig(bos,19)=0; end;
  if ~isempty(scr_ysize),  if typ=='axes'; bose=I2Mfig(fii,2); else bose=base;  end;  bos=int16(bose);
                                                                pos=I2Mfig(bos,7:10); pos(4)=scr_ysize;  I2Mfig(bos,7:10)=pos; set(bose,'position',pos); I2Mfig(bos,20)=0; end;

function [wid,varargout]=widget_label(varargin)
%FUNCTION widget_label, base, ...
%***  ****************
%***
%Unused: dynamic_resize

    I2Mkwn=char('I2M_a1' , 'notify_realize' , 'func_get_value' , 'pro_set_value' , 'map' , 'value' , 'uvalue' , 'font' , 'xsize' , 'ysize' , 'scr_xsize' , 'scr_ysize' , 'sensitive' , 'uname' , 'resource_name' , 'dynamic_resize' , 'I2M_pos' );
    I2Mkwv=    {'base'   , 'not_r'          , 'f_get'          , 'p_set'         , 'map' , 'vol'   , 'uval'   , 'font' , 'xsize' , 'ysize' , 'scr_xs'    , 'scr_ys'    , 'sens'      , 'uname' , 'resource_name' , 'dyn'            , 'I2M_pos' };
    base=[]; not_r=[]; f_get=[]; p_set=[]; map=[]; vol=[]; uval=[]; font=[]; xsize=[]; ysize=[]; scr_xs=[]; scr_ys=[]; sens=[]; uname=[]; resource_name=[]; dyn=[]; I2M_pos=[];
...
wid=uicontrol(fig,'style','text','position',pos,'string',val,'userdata',uval); fii=double(int16(wid));

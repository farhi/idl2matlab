...
%colors, frame, graphics_level, motion_events(figure)
    ...
    I2Mkwv=    {'base'   , 'not_r'          , 'eventp'    , 'eventf'     , 'f_get'          , 'p_set'         , 'map' , 'uval'   , 'xsize' , 'ysize' , 'src_xs'    , 'scr_ys'    , 'x_scr'         , 'y_scr'         , 'sens'      , 'uname' , 'but_ev'        , 'I2M_pos' };
    base=[]; not_r=[]; eventp=[]; eventf=[]; f_get=[]; p_set=[]; map=[]; uval=[]; xsize=[]; ysize=[]; scr_xs=[]; scr_ys=[]; x_scr=[]; y_scr=[]; sens=[]; uname=[]; but_ev=[]; I2M_pos=[];
...
%
wi0=axes('Parent',fig,'units','pixels','position',pos,'FontSize',8,'FontWeight','bold','NextPlot','replace','userdata',uval,'color','black','xtick',[],'ytick',[],'box','on');
fii=double(int16(wi0));
if ~mop;  set(wid,'visible','off'); end;
if ~sens; set(wid,'enable' ,'off'); end;
I2Mfig(fii,1)   =wi0; I2Mfig(fii, 2)=wwc;  I2Mfig(fii, 3)=fig;
I2Mfig(fii,4)   =3;   I2Mfig(fii, 5)=map;  I2Mfig(fii, 6)=2;
I2Mfig(fii,7:10)=pos; I2Mfig(fii,15)=sens; I2Mfig(fii,16)=but;
I2Mfig(fii,11)  =I2Mfig(ibas,11);          I2Mfig(fii,17)=0;
I2Mfig(fii,12)  =I2Mfseq;                  I2Mfig(fii,18)=0;
I2Mfun{fii}='';                            I2Mfig(fii,19:20)=0;
I2Mfseq=I2Mfseq+1;

wid=axes('Parent',fig,'units','pixels','position',pos,'FontSize',8,'FontWeight','bold','NextPlot','replace','userdata',uval,'color','black','xtick',[],'ytick',[],'box','on');
...
I2Mfig(fii,11)  =I2Mfig(ibas,11);          I2Mfig(fii,17)=0;
I2Mfig(fii,12)  =I2Mfseq;                  I2Mfig(fii,18)=0;
I2Mfun{fii}=char(uname);                   I2Mfig(fii,19:20)=0;

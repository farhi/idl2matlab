function [wid,varargout]=widget_droplist(varargin)
%FUCTION widget_droplist, base, ...
%*** *******************
%***

    I2Mkwn=char('I2M_a1' , 'notify_realize' , 'event_pro' , 'event_func' , 'func_get_value' , 'pro_set_value' , 'map' , 'value' , 'uvalue' , 'font' , 'xsize' , 'ysize' , 'scr_xsize' , 'scr_ysize' , 'sensitive' , 'title' , 'uname' , 'resource_name' , 'I2M_pos' );
    I2Mkwv=    {'base'   , 'not_r'          , 'eventp'    , 'eventf'     , 'f_get'          , 'p_set'         , 'map' , 'vol'   , 'uval'   , 'font' , 'xsize' , 'ysize' , 'scr_xs'    , 'scr_ys'    , 'sens'      , 'title' , 'uname' , 'resource_name' , 'I2M_pos' };
    base=[]; not_r=[]; eventp=[]; eventf=[]; f_get=[]; p_set=[]; map=[]; vol=[]; uval=[]; font=[]; xsize=[]; ysize=[]; scr_xs=[]; scr_ys=[]; sens=[]; edi=[]; uname=[]; resource_name=[]; I2M_pos=[];
...
wid=uicontrol(fig,'style','popupmenu','position',pos,'string',val,'userdata',uval,'value',1); fii=double(int16(wid));
...
ev=['struct(''structure_name'',''WIDGET_DROPLIST'',''id'',' num2str(fii) ',''index'',v)'];
set(wid,'Callback',['wid=' num2str(wid,20) '; v=get(wid,''value'')-1; widget_i2mevent(' ev ');']);

...
%   rectangular submatrix of A specified by the subscript vectors I and J. 
%   B must have LENGTH(I) rows and LENGTH(J) columns.  A colon used as a
%   subscript, as in A(I,:) = B, indicates the entire column (or row).
%
%   For multi-dimensional arrays, A(I,J,K,...) = B assigns B to the
%   specified elements of A. B must be LENGTH(I)-by-LENGTH(J)-by-LENGTH(K)-...
...
%   $Revision: 1.13 $  $Date: 2001/04/15 12:00:22 $
%   Built-in function.
...
if S(1).type(1) == '.';  A=builtin('subsasgn',A,S,B); else;
if isa(B,'i2mstr'), A.cell=builtin('subsasgn',A.cell,S,B.cell);
else                A.cell=builtin('subsasgn',A.cell,S,cellstr(B)); end;
end

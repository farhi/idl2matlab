...
global i2mDecomposed i2mvs_x i2mvs_y i2mvs_z i2mvs_p
	...
	switch index(1).subs
		...
		case 'charsize'
			...
			if p.font ~= 0
				...
				if (val > 0)
					set(0,'DefaultAxesFontSize',10*val);
					set(0,'DefaultTextFontSize',10*val);
				else
					% cas ou val <= 0
		...
		case 'noerase'
			...
			% hold on;
			% else
			% 	set(0,'DefaultFigureNextplot','replacechildren');
			% 	set(0,'DefaultAxesNextplot','replacechildren');
		...
		case 'background'
			p.background = val;
			if i2mDecomposed == 0
				% On utilise la table de couleurs
				c = i2mColormap;
				col = c(val+1,:);
			else
				% On n'utilise pas la table de couleurs
				col = i2m_index2rgb(val);
			end

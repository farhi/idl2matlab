...
if ~isempty(fmt), fmtc=i2m_format(fmt); else fmtc=''; end;
if ~isempty(str);
    if ~ischar(str) & ~iscellstr(str);
        if isnumeric(str);  if ~strcmp(class(str),'uint8'), str=num2cell(str); else, str=char(str); end; end;
        if iscell(str); for i=prod(size(str)):-1:1; if isnumeric(str{i});
                            if isempty(fmtc), str{i}=num2str(str{i});
                            else,  try,       str{i}=num2str(str{i},fmtc); catch, str{i}=num2str(str{i}); end; end;
                        end; end;

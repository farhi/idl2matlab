...
I2Mkwn=char('close' ,'set_resolution','get_screen_size','window_state','get_window_position','decomposed','get_decomposed','retain','get_graphics_function', 'set_graphics_function','copy' ,'filename','color'  ,'inches','landscape','portrait','close_file',...
	...
	'tty','user_font','vt240','vt241','vt340','vt341','xoffset','xon_xoff','xsize','yoffset','ysize','zapfchancery','zapfdingbats','z_buffering','font','I2M_pos');
...
I2MnotTranslated = char('avantgarde','average_lines','binary','bits_per_pixel','bkman','bold','book','bypass_translation','close_document','colors','courier','cursor_crosshair','cursor_image','cursor_mask','cursor_original','cursor_standard','cursor_xy','demi','depth','direct_color',...
	'eject','encapsulated','encoding','floyd','font_index','font_size','get_current_font','get_fontnames','get_fontnum','get_page_size','get_visual_depth','get_visual_name','get_write_mask','gin_chars','glyph_cache','helvetica','index_color',...
	'isolatin1','italic','light','medium','narrow','ncar','oblique','optimize','ordered','output','palatino','pixels','plot_to','plotter_on_off','polyfill','pre_depth','pre_xsize','pre_ysize','print_file','pseudo_color','reset_string','resolution',...
	'scale_factor','schoolbook','set_character_size','set_colormap','set_colors','set_font','set_string','set_translation','set_write_mask','static_color','static_gray','symbol','tek4014','tek4100','text','threshold','times','translation','true_color','tt_font',...
	'tty','user_font','vt240','vt241','vt340','vt341','xon_xoff','zapfchancery','zapfdingbats','z_buffering','font');

	% Variables utilisees
	pos = [];
	% variables SYSTEME
	global i2mvs_d i2mZbuff i2mPs i2mDecomposed i2mvs_p i2mFilename i2mDevPsColor i2mDevPsPreview i2mPsCanWrite

	% initialisation des valeurs des keywords specifiques
	closeZ =''; set_resolution = []; window_state = 0.128; get_screen_size = 0.128; window_position = 0.128; decomposed = '';get_decomposed = 0.128; retain = '';set_graphics_function='';copie='';get_graphics_function=0.128;filename='';couleur='';inches='';
	landscape='';portrait=''; close_file='';xoffset='';yoffset='';xsize='';ysize='';preview=''; I2M_pos=[];

	% initialisation des valeurs des keywords non traduits.
	for i=1:length(I2MnotTranslated), eval( [I2MnotTranslated(i,:) '= '''';']); end;

	I2M_pos=[]; I2M_lst={}; I2M_out=''; lv=length(varargin); if rem(lv,2) ~= 0, I2M_ok=0; else, I2M_ok=1;
for I2M=1:2:lv; I2M_tmp=varargin{I2M}; if ~ischar(I2M_tmp); I2M_ok=0; break; end; I2Mx=strmatch(I2M_tmp,I2Mkwn); if length(I2Mx) >1, I2Mx=strmatch(I2M_tmp,I2Mkwn,'exact'); end; if length(I2Mx) ~=1,if I2M==1, I2M_ok=0; break; end; else, eval([I2Mkwv{I2Mx} '=varargin{I2M+1};']); I2M_lst{(I2M+1)/2}=I2Mkwv{I2Mx}; end; end; end;
...
if isempty(window_state) | (window_state ~= 0.128)
...
if isempty(get_screen_size) | (get_screen_size ~= 0.128)
	if strcmpi(i2mvs_d.name,'X') | strcmpi(i2mvs_d.name,'WIN') | strcmpi(i2mvs_d.name,'MAC')
		% Bon device courant
		set(0,'Units','pixels');
...
if isempty(window_position) | (window_position ~= 0.128)
	if strcmpi(i2mvs_d.name,'X') | strcmpi(i2mvs_d.name,'WIN') | strcmpi(i2mvs_d.name,'MAC')
		% Bon device courant
		set(gcf,'Units','pixels');
		wpos=[0,0];
		if i2mvs_d.window > 0, wPos = get(i2mvs_d.window,'position'); end
...
if isempty(get_decomposed) | (get_decomposed ~= 0.128)
	if strcmpi(i2mvs_d.name,'X') | strcmpi(i2mvs_d.name,'WIN') | strcmpi(i2mvs_d.name,'MAC')
		% Bon device courant
	...
	if strcmpi(i2mvs_d.name,'X') | strcmpi(i2mvs_d.name,'WIN') | strcmpi(i2mvs_d.name,'MAC')
		...
		if (decomposed <= -1) | (decomposed >= 1)
			if i2mDecomposed ~= 1
				...
				i2mDecomposed = 1; ok=1;
			end
		else
			if i2mDecomposed ~= 0
				% On n'etait pas dans ce mode
				i2mDecomposed = 0; ok=1;
			end
		end
		if ok
		% On va mettre a jour les composantes RGB des couleurs par defaut :
		% pour !P.COLOR
		col = i2mColormap(i2mvs_p.color);
...
if isempty(get_graphics_function) | (get_graphics_function ~= 0.128)
	if strcmpi(i2mvs_d.name,'X') | strcmpi(i2mvs_d.name,'WIN') | strcmpi(i2mvs_d.name,'MAC')
		% Bon device courant
...
if ~isempty(copie)
	if strcmpi(i2mvs_d.name,'X') | strcmpi(i2mvs_d.name,'WIN') | strcmpi(i2mvs_d.name,'MAC')
		...
		if length(copie) >= 6,
			...
			xd      = copie(5);
			yd      = copie(6);
			if i2mvs_d.window > 0, currAx  = i2mvs_d.window; else currAx=gca; end;
		end;
		if length(copie) == 7,
		        wset(copie(7));                           % La fenetre depuis laquelle on va copier les pixels a ete specifiee
			pic = tvrd(cLeft,cBottom,cWidth,cHeight); % On copie les pixels
			wset(currAx);                             % On se replace dans la fenetre courante
			tv(pic,xd,yd);
		elseif length(copie) == 6
		        wset(currAx);
		        pic = tvrd(cLeft,cBottom,cWidth,cHeight);
		        tv(pic,xd,yd);
                else,   error('Keyword array parameter COPY must have from 6 to 7 elements.')
			end
	else
		% Mauvais device courant
...
% Traitement du keyword /PREVIEW
if ~isempty(preview)
	if strcmpi(i2mvs_d.name,'PS')
		% Bon device courant
		i2mDevPsPreview = preview;
	else
		% Mauvais device courant
		error( 'Keyword PREVIEW not allowed in call to: DEVICE. Current device should be ''PS''');
	end
end
...
% Traitement des keywords /XOFFSET, /YOFFSET, /XSIZE, /YSIZE
if ~isempty(xoffset) | ~isempty(yoffset) | ~isempty(xsize) | ~isempty(ysize)
	if strcmpi(i2mvs_d.name,'HP') | strcmpi(i2mvs_d.name,'PS') | strcmpi(i2mvs_d.name,'LJ') | strcmpi(i2mvs_d.name,'PCL') | strcmpi(i2mvs_d.name,'PRINTER')
		% Bon device courant
		if strcmpi(i2mvs_d.name,'PS')
			% !D.NAME = 'PS', le device courant est 'PS'
			paperPos = get(i2mPs,'PaperPosition');
			gauche = paperPos(1); % xoffset par defaut
			bas = paperPos(2); % yoffset par defaut
			hauteur = paperPos(4); % height A4 par defaut
			longueur = paperPos(3); % width A4 par defaut
			if ~isempty(xoffset)
				gauche = xoffset;
			end
			if ~isempty(yoffset)
			...
			set(i2mPs,'PaperPosition',[gauche,bas,longueur,hauteur]);
		else
			warning('Translation of keyword XOFFSET for the current device is not yet implemented');
		end
	else
		% Mauvais device courant
		error( 'Keyword XOFFSET not allowed in call to: DEVICE. Current device should be ''HP'', ''PS'', '' LJ'', ''PCL'' or ''PRINTER''');
	end
end

...
if strcmpi(dev,'X') | strcmpi(dev,'WIN')
	...
	if ~isempty(i2mOldGcf)
		...
		% vieux commentaire : On est oblige de creer une figure pour que la figure courante soit differente de celle du device precedent
		% vieux code : window;
		% vieux code : i2mOldGcf = i2mvs_d.window;
	end
end
...
global i2mZbuff i2mPs i2mvs_d i2mOldGcf

...
I2Mkwn=char('I2M_a1' , 'I2M_a2' , 'I2M_a3', 'alignment' , 'charsize'  , 'charthick' , 'text_axes' , 'width' ,'orientation' , 'color', 't3d' ,'I2M_pos');
    I2Mkwv=    {'d1'     , 'd2' , 'texte' , 'align'     , 'taillecar' , 'epaisscar' , 'axes_texte', 'taille',  'angle','colorIndice','t3dKG','I2M_pos'};

	% Variables utilisees
	% variables SYSTEME
	global i2mvs_p coefTailleCar
	...
	if n_elements(texte) == 1
		...
		for i=1:mini
			...
			if length(colorIndice) <= 1
				GestionKeywords (txt,1,colorIndice,align,taillecar,epaisscar,axes_texte,angle,taille,t3dKG);
			else
				if length(colorIndice) >= mini
					GestionKeywords (txt,i,colorIndice,align,taillecar,epaisscar,axes_texte,angle,taille,t3dKG);
				else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [varargout] = GestionKeywords (txt,i,colorIndice,align,taillecar,epaisscar,axes_texte,angle,taille,t3dKG)
...
if ~isempty(taillecar)
	if i2mvs_p.font ~= 0
		...
		if (taillecar > 0)
			set(txt,'FontSize',coefTailleCar*taillecar);
		else
			% cas ou taillecar <= 0
			if taillecar == 0
				set(txt,'FontSize','default')
			else
...
if isempty(taille) | (taille ~= 0.128)

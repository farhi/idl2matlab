...
%   $Revision: 1.12 $  $Date: 2001/04/15 12:00:30 $
%   Built-in function.
...
if S(1).type(1) == '.'; B=builtin('subsref',A,S); else;

S.type='()';
if length(S.subs) == 1, if ischar(S.subs{1}), if S.subs{1} == ':',
   B=A.cell; if length(B) == 1,  B=B{1}; end; return; end, end, end;

B=builtin('subsref',A.cell,S);

if length(B)  > 1, sz=size(B); if sz(2) ==1, B=B'; end; B=i2mstr(B); end;
if length(B) == 1, if iscell(B), B=B{1}; end; end;

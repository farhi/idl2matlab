function [wid,varargout]=widget_list(varargin)
%FUCTION widget_list, base, ...
%*** ***************
%***

    I2Mkwn=char('I2M_a1' , 'notify_realize' , 'event_pro' , 'event_func' , 'func_get_value' , 'pro_set_value' , 'map' , 'value' , 'uvalue' , 'font' , 'xsize' , 'ysize' , 'scr_xsize' , 'scr_ysize' , 'sensitive' , 'multiple' , 'uname' , 'resource_name' , 'I2M_pos' );
    I2Mkwv=    {'bise'   , 'not_r'          , 'eventp'    , 'eventf'     , 'f_get'          , 'p_set'         , 'map' , 'vol'   , 'uval'   , 'font' , 'xsize' , 'ysize' , 'scr_xs'    , 'scr_ys'    , 'sens'      , 'multi'    , 'uname' , 'resource_name' , 'I2M_pos' };
    bise=[]; not_r=[]; eventp=[]; eventf=[]; f_get=[]; p_set=[]; map=[]; vol=[]; uval=[]; font=[]; xsize=[]; ysize=[]; scr_xs=[]; scr_ys=[]; sens=[]; multi=[]; uname=[]; resource_name=[]; I2M_pos=[];
...
if scr_xs; xsz=scr_xs; szz(1)=0; elseif xsize,  xsz=(xsize+3)*I2Mfcar(1); else, xsz=lx; end;
if scr_ys; ysz=scr_ys; szz(2)=0; elseif ysize,  ysz=(ysize  )*I2Mfcar(2); else, ysz=ly; end;
pos=[0  0  xsz  ysz];

if multi, mx=ly; else, mx=1; end;
wid=uicontrol(fig,'style','listbox','min',0,'max',mx,'position',pos,'string',val,'HorizontalAlignment','left','userdata',uval); fii=double(int16(wid));
...
ev=['struct(''structure_name'',''WIDGET_LIST'',''id'',' num2str(fii) ',''index'',ind,''clicks'',2)'];
set(wid,'Callback',['wid=' num2str(wid,20) '; ind=get(wid,''value'')-1; widget_i2mevent(' ev ');']);

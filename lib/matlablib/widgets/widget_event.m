...
%UNUSED : yield_to_tty
    ...
    I2Mkwv=    {'bise'   , 'bad_id' , 'nowait' , 'save_hourglass' , 'yield'        , 'I2M_pos'};
    bise=[]; bad_id=[]; nowait=[]; save_hourglass=[]; yield=[]; I2M_pos=[];
...
bad_id=0;
if isempty(bise)          , base          =0; else, base=I2Mfig(int16(bise),1); end;
...
%if ~nowait, waitfor(base(1),'Callback'); end;
 if ~nowait, uiwait (I2Mfig(fii(1),3));   end;

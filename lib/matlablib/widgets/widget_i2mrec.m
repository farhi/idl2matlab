...
%*********************
%**
global I2Mfig I2Mfevn I2Mnotr I2Mreg I2Mfgap I2Mfcar
...
switch flag;
case 'del';   if ishandle(wid), delete(wid); end;
              if I2Mfig(fii,6) == 3; if ishandle(I2Mfig(fii,3)), delete(I2Mfig(fii,3)); end;
...
case 'map';   if I2Mfig(fii,6) == 2; if I2Mfig(fii, 5); try, tmp=get(wid,'type'); if (tmp ~= 'uicontextmenu'), set(wid,'visible','on');  end; catch; end; end; end;
              if I2Mfig(fii,6) == 3; if I2Mfig(fii, 5); try, set(I2Mfig(fii,3),'visible','on'); catch; end; end; end;
case 'nomap'; if I2Mfig(fii,6) == 2;                    try, tmp=get(wid,'type'); if (tmp ~= 'uicontextmenu'), set(wid,'visible','off'); end; catch; end; end;
              if I2Mfig(fii,6) == 3;                    try, set(I2Mfig(fii,3),'visible','off'); catch; end; end;
case 'sens';  if I2Mfig(fii,6) == 2; if I2Mfig(fii,15); try, tmp=get(wid,'type'); if (tmp ~= 'uicontextmenu'), set(wid,'enable' ,'on');  end; catch; end; end; end;
case 'nosens';if I2Mfig(fii,6) == 2;                    try, tmp=get(wid,'type'); if (tmp ~= 'uicontextmenu'), set(wid,'enable','off');  end; catch; end; end;
...
case 'revers';if I2Mfig(fii,6) ~= 0; widget_i2mpos(fii,  0, 'revers');
                                     ...
                                     if tmp == 'axes',
                                        if I2Mfig(fii,21) > 0, 
                                            I2Mfig(fii,7:10)=I2Mfig(fii,7:10)+[I2Mfcar(1)*8,I2Mfcar(2)*3,-I2Mfcar(1)*10,-I2Mfcar(2)*5];
                                        end;
                                     end;
                                     if I2Mfig(fii,9)  < 1, I2Mfig(fii,9) =1; end;
                                     if I2Mfig(fii,10) < 1, I2Mfig(fii,10)=1; end;
                                     set(wid,'position',I2Mfig(fii,7:10)); 
                                     end; catch; end; end;
case 'event';
otherwise;
end;

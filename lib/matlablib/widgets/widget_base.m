...
%UNUSED : scroll, xsize,ysize, scr_xsize,scr_ysize, kbrd_focus, modal
    ...
    I2Mkwv=    {'bise'   , 'titl'  , 'row' , 'col'    , 'not_r'          , 'eventp'    , 'eventf'     , 'f_get'          , 'p_set'         , 'uval'   , 'xsize' , 'ysize' , 'sens'      , 'map' , 'frame' , 'exclusive' , 'nonexclusive' , 'kill_notify' , 'group_leader' , 'resource_name' , 'tlb_size_events' , 'tlb_kill'                , 'uname' , 'mbar' , 'I2M_pos'};
    bise=[]; titl=[]; row=[]; col=[]; not_r=[]; eventp=[]; eventf=[]; f_get=[]; p_set=[]; uval=[]; xsize=[]; ysize=[]; sens=[]; map=[]; frame=[]; exclusive=[]; nonexclusive=[]; kill_notify=[]; group_leader=[]; resource_name=[]; tlb_size_events=[]; tlb_kill=[]; uname=[] ; mbar=[]; I2M_pos=[];
...
global I2Mfig I2Mfevn I2Msget I2Mnotr I2Mfseq I2Mfgap I2Mfcar I2Mftim I2Mfun i2m_wcolor i2m_wcolorOn
...
if  isempty(frame) | i2m_wcolorOn; visibl='off'; else, visibl='on';   end;
uic=1;
if isempty(bise); if isempty(titl); titl=''; if map == 0, titl='unvisible'; end; end;
    if isempty(I2Mfseq); I2Mfseq=1; I2Mftim=0; I2Mfun=cellstr(['','','']); end;
    cocol=i2m_wcolor; if ~i2m_wcolorOn, cocol(2)=cocol(2)-.2; end;
    fig=figure('name',titl,'visible','off','numbertitle','off','menubar','none','color',cocol,'BusyAction','queue','Interruptible','off','unit','pixels');
    uic=3; parent=fig;
else; base=I2Mfig(int16(bise),1); parent=base; ibas=int16(parent); fig=I2Mfig(ibas,3); bgc=I2Mfig(ibas,18); end;

wid=uicontrol(fig,'style','frame','visible',visibl,'userdata',uval); fii=double(int16(wid));
%wi0=axes('Parent',fig,'units','pixels','position',pos,'HitTest','off','xtick',[],'ytick',[],'box','off','xlim',[1 xsz],'ylim',[1 ysz]);
...
I2Mfig(fii,17)  =grp; I2Mfig(fii,18)=bgc;    I2Mfig(fii,19:20)=0; I2Mfig(fii,21)=0;

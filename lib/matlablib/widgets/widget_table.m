...
%TODO   : column_labels, row_labels, (select in control)
    ...
    I2Mkwv=    {'bise'   , 'not_r'          , 'col_sx'        , 'row_sy'      , 'col_lab'       , 'row_lab'    , 'col_maj'      , 'row_maj'   , 'frame' , 'no_headers' , 'eventp'    , 'eventf'     , 'f_get'          , 'p_set'         , 'map' , 'vol'   , 'uval'   , 'font' , 'xsize' , 'ysize' , 'scr_xs'    , 'scr_ys'    , 'sens'      , 'edi'      , 'uname' , 'resource_name' , 'I2M_pos' };
    bise=[]; not_r=[]; col_sx=[]; row_sy=[]; col_lab=[]; row_lab=[]; col_maj=[]; row_maj=[]; frame=[]; no_headers=[]; eventp=[]; eventf=[]; f_get=[]; p_set=[]; map=[]; vol=[]; uval=[]; font=[]; xsize=[]; ysize=[]; scr_xs=[]; scr_ys=[]; sens=[]; edi=[]; uname=[]; resource_name=[]; I2M_pos=[];

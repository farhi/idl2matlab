...
% --------------------------------------------------------
% Fonction : fonction i2m_tr
%            pour traduire une commande simple IDL
% Auteurs :
%                 Didier Richard
...
global I2M_tr_putend I2M_tr_mtc I2M_tr_block I2M_tr_incas I2M_tr_rename

if length(I2M_tr_mtc) ==0, I2M_tr_mtc='[(,=:*#+-/\><'; I2M_tr_block=0; I2M_tr_incas=0;
                           I2M_tr_rename={}; end;
...
%!  cmd=strrep(cmd,' cloze '       ,' M2I_cloze ');	    cmd=strrep(cmd,' close ,'       ,' cloze ,'); %!
    ...
    cmd=strrep(cmd,' plott '       ,' M2I_plott ');	    cmd=strrep(cmd,' plot ,'        ,' plott ,');
    cmd=strrep(cmd,' surfacee '    ,' M2I_surfacee ');  cmd=strrep(cmd,' surface ,'     ,' surfacee ,');
    cmd=strrep(cmd,' contourr '    ,' M2I_contourr ');  cmd=strrep(cmd,' contour ,'     ,' contourr ,');
    cmd=strrep(cmd,' axxis '       ,' M2I_axxis ');     cmd=strrep(cmd,' axis ,'        ,' axxis ,');
...
%--------------------
    ...
    cmd=strrep(cmd,' var '         ,' M2I_var ');
    cmd=strrep(cmd,' line '        ,' M2I_line');
%sys variables
%-------------

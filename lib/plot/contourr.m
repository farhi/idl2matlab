...
I2Mkwn=char('I2M_a1' , 'I2M_a2' , 'I2M_a3' ,'irregular' ,'triangulation' ,'downhill' ,'follow' ,'overplot' ,'max_value' ,...
    ...
    'ticklen','background','color','xminor','yminor','zminor','nodata' ,'noerase' ,'xstyle' ,'ystyle' , 'clip', 'data',...
		'font','noclip','normal','subtitle','t3d','thick','xcharsize','ycharsize','zcharsize','xgridstyle','ygridstyle','zgridstyle','I2M_pos');
I2Mkwv=    {'initd1' , 'initd2' , 'initd3' ,'irregularr','triangulationn','downhilll','followw','multiplot','maxi'      ,...
    ...
    'ticlen' , 'backColor','colorIndice','xminorTick','yminorTick','zminorTick','nodata','noerase','xstylee','ystylee','clip', 'data',...
		'police','noclip','normal','subtitle','t3d','lthick','xcharsize','ycharsize','zcharsize','xgridstyle','ygridstyle','zgridstyle','I2M_pos'};

% variables utilisées
txt='';c='';plus='';
% variables globales
global i2mDecomposed
...
nodata='';xstylee='';ystylee='';noerase='';clip='';data='';police='';noclip='';normal='';subtitle='';t3d='';lthick='';xcharsize='';ycharsize='';zcharsize='';xgridstyle='';ygridstyle='';zgridstyle='';
...
if ~(length(colors)==0)
    % On recupere la colormap courante
		c =[];
		if i2mDecomposed == 0
			c=i2mColormap;
		end
    ...
    for i=1:length(h)
        ...
        if ( ~isempty(rempli) | ~isempty(rempliBis) )
						if i2mDecomposed == 0
							set(h(i),'FaceColor',c(colors(j)+1,:));
						else
							set(h(i),'FaceColor',i2m_index2rgb(colors(j)));
						end
        end;
				if i2mDecomposed == 0
	        set(h(i),'EdgeColor',c(colors(j)+1,:));
				else
					set(h(i),'EdgeColor',i2m_index2rgb(colors(j)));
				end
        j=j+1;
    end;
end;
...
% Traitement de /T3D
i2m_t3d(t3d);
% Traitement de [XYZ]CHARSIZE
i2m_xyzcharsize(xcharsize,ycharsize,zcharsize);
% Traitement de [XYZ]GRIDSTYLE
i2m_xyzgridstyle(xgridstyle,ygridstyle,zgridstyle);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (~isempty(multiplot) | ~isempty(downhilll))
    hold off;
end;
